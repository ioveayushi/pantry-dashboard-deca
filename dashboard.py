...
# ---------- CLI args (streamlit passes args after `--`) ----------
def get_args():
...
model = joblib.load(ARGS.model) if ARGS.model else joblib.load("model.joblib")


# ---------- Load data ----------
data_path = ARGS.data
if not data_path:
    st.error("No --data path passed. Run via run_pipeline.py or launch with: streamlit run dashboard.py -- --data <file> --model <file> --metrics <file>")
    st.stop()
...
if ARGS.metrics:
    st.caption(f"Metrics: {ARGS.metrics}")


# ---------- Choose pantry + build “latest row” ----------
pantry_ids = sorted(df["pantry_id"].unique().tolist())
default_pid = pantry_ids[0] if pantry_ids else 0
...
with left:
    ...
    pantry_id = st.selectbox("Pantry ID", pantry_ids, index=pantry_ids.index(default_pid) if default_pid in pantry_ids else 0)
    ...
    with c5:
        ...
        econ_pressure_index = st.number_input("Economic pressure (0–1.5)", min_value=0.0, max_value=1.5, value=float(econ_pressure_index))
    with c6:
...
with right:
    ...
    # Auto-run on load too (so it immediately shows updates)
    if run or True:
        preds = predict()
